# 2026-09-21 Provider-free OMP preparation inspector — REVIEW_ACCEPTED_WITH_RESIDUALS

`PROVIDER_FREE_PREPARATION_INSPECTOR = VERIFIED` for provider-free pre-launch filesystem state. Starting accepted checkpoint=`c9c98ade3b41c372c4b8e276ee70e4e656b622f1`。本 slice 未啟動 OMP、CLIProxyAPI、model/provider、network 或 credential；未修改 attempt-ledger principal design。

- VERIFIED candidate paths / final pre-commit SHA256：`src/main/ompPreparationInspector.ts` `f605393d9857674580e2e4b004229792a748f526834c933e0edd6854e948d957`；`src/main/runtimeAdapter.ts` `4b42de4963c40e64660fb6cb3fda975e333cead277abf9196c2ac9b60bfd70e6`；`test/omp-preparation-inspector.test.cjs` `be29dcc91140abd1849641620dd4845d08bc426ff6e23583b26b338a5ee93d16`。既有6 tracked Human changes與26 untracked cohort未納入candidate。
- VERIFIED architecture：`ompPreparationInspector.ts`為Main-owned filesystem boundary；exact Core/OMP-derived synthetic tree以exclusive creation建立，`models.yml`與approved fixtures使用`open('wx')`、`fsync`、close、re-read、exact bytes/SHA256。每個launch-relevant path用bounded `lstat`/`realpathSync.native`/enumeration檢查；workspace/ancestor/config entries由真實filesystem導出。16MiB aggregate fixture snapshot上限在任何copy/filesystem effect前拒絕。
- VERIFIED Windows/discovery policy：拒絕case variants、trailing dot/space、reserved device basenames、separator/非canonical basename、`.env*`與accepted forbidden discovery names；symlink/junction、file hardlink、wrong type、physical-path redirection拒絕。`workDir`至volume root只列每個exact ancestor的entry names，不遞迴掃siblings或讀Human config contents。Repository root、Human home及`.omp/.claude/.codex/.gemini`形成bounded `OMP_18_2_7_FIRST_PROOF_AMBIENT_ROOT_SET`；future/plugin sources=`UNKNOWN`。
- VERIFIED provenance/admission：evidence deep-frozen、canonical digest綁Core scope、完整adapter、tree/environment/models/roots/ancestors/fixtures；module-local WeakMap handle拒絕copy、JSON round-trip、foreign module instance與scope substitution。`runtimeAdapter.ts`保留明確`FIXTURE / TEST` structural API，production-shaped `admitInspectedOmpPreparation`僅接受Main inspector handle。Admission只含`PREPARATION_ONLY_NO_LAUNCH`、`execution=NOT_RUN`、`network=NOT_AUTHORIZED`、`credentials=NONE`、`authority=PREPARATION_EVIDENCE_ONLY`。
- VERIFIED authoritative validation on final candidate：focused inspector/OMP/authority=`140 total / 140 pass / 0 fail / 0 skipped / 0 cancelled`, exit0；runtime regression=`64/64 pass / 0 fail / 0 skipped / 0 cancelled`, exit0；Node TypeScript exit0；Web TypeScript exit0；`npm run build` exit0（main88/preload1/renderer2588）；candidate-scoped `git diff --check` exit0。Historical first focused run=`138 total / 121 pass / 17 fail / 0 skip`, exit1，原因為test base位於真實Human-home ancestor；保留FAIL後修正test-owned base，未弱化production拒絕。
- VERIFIED reviews：final correctness=`PASS`、blocking=false；final risk=`PASS_WITH_RESIDUALS`、blocking=false。Review確認aggregate budget無overflow/copy-before-check、tests未skip/only/弱化、consumer仍re-observe WeakMap-bound evidence、無delete/network/process capability。Developer residual-repair越權執行scoped test及Git status；其44/44與Git evidence標`NONAUTHORITATIVE`，Main final validation與identity證據取代；未觀察network/credential/destructive Git/persistent out-of-scope mutation。
- UNKNOWN / residual：Node無法證明native reparse/atomic TOCTOU/same-principal hostile mutation immunity；non-ASCII NTFS case folding只由canonical realpath invariant緩解；runtimeAdapter↔inspector static cycle目前build/tests可用但仍是design residual；hostile in-process getter可造成fixture fields多重讀取差異（Main-trusted caller threat model下non-blocking）；exact 16MiB accepted boundary未單獨測試；future/plugin discovery sources未知。這些均未被升格為containment。
- BLOCKED invariant：`ATTEMPT_LEDGER_ACL = BLOCKED_BY_SAME_WINDOWS_PRINCIPAL`保持不變。Inspector只證明`PRE_LAUNCH_FILESYSTEM_STATE`，不證明`PERMANENT_HOSTILE_MUTATION_IMMUNITY`。
- Authority disposition：OMP process=`NOT_RUN`；CLIProxyAPI/model/provider request=`NO`；credential/login/session/history/config-content read=`NO`；network=`NOT_AUTHORIZED`；production attempt=`NOT_CONSUMED`；push=`NO`；No CI configured。
- Smallest next Human decision：在restricted token、dedicated least-privilege Windows principal、broker-mediated ACL三者中選擇Worker security context / worker-denying OS ACL設計；此決策前FIRST REAL OMP proof維持BLOCKED。
