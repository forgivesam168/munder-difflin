...
function document() {
 ...
 const window={location:{reload(){assert.fail('Renderer navigation must not be used');}},cth:{controlledReload(){reloads++;return Promise.resolve();},controlledReadProject(){calls.push('project');return Promise.resolve({projectRoot:'synthetic'});},controlledReadReady(){calls.push('ready');return new Promise(resolve=>{readyResolve=resolve;});},readFile(){calls.push('read');return new Promise(resolve=>{readResolve=resolve;});},controlledReadDisplayed(){calls.push('display');return Promise.resolve();}}};
