...
const candidate={candidateSha256:'a'.repeat(64)};
...
const good={version:1,runId:binding.runId,candidateSha256:req.candidateSha256,nonce:req.nonce,requestSha256:binding.digest(req),result:'PASS',events:[...A1_SEQUENCE]};
function transport(accept=()=>{}) {
  ...
  const pending=require('../tools/windows-lifecycle-transport.cjs')({powershell:'fixed',helperArgs:[],run:'run',env:{},receipt,persist(){},accept,mode:{success:'A1_PASS'},outerMs:115000,spawn:()=>child,
...
test('task sequence requires both fresh consents, read deliveries, renderer acknowledgements and normal close',()=>{
 for(const omit of [-1,...A1_SEQUENCE.map((_,i)=>i)]){let actual;const state=new ControlledAcceptance(r=>actual=r);A1_SEQUENCE.forEach((e,i)=>{if(i!==omit)state.observe(e);});state.finish(true);assert.equal(actual.result,omit===-1?'PASS':'FAIL');}
 for(const reason of ['timeout','bad-content','repeat']){let actual;const state=new ControlledAcceptance(r=>actual=r);for(const e of A1_SEQUENCE)state.observe(e);if(reason==='bad-content')state.display('wrong');if(reason==='repeat')state.observe('read');state.finish(reason!=='timeout');assert.equal(actual.result,'FAIL');}
});
test('bound result rejects stale candidate/nonce/request, partial and supervisor-only PASS',()=>{
 binding.validateResult(good,req);
 for(const delta of [{candidateSha256:'d'.repeat(64)},{nonce:'d'.repeat(64)},{requestSha256:'d'.repeat(64)},{events:[]},{result:'INCOMPLETE'},{version:2}])assert.throws(()=>binding.validateResult({...good,...delta},req));
...
test('real product result producer writes request-bound receipt only on completed close',t=>{
 ...
 assert.ok(!fs.existsSync(path.join(appData,'a1-result.json')));producer.finish(true);
...
test('115s outer timeout targets only retained helper and fallback never establishes cleanup',async()=>{
 const h=transport();[...h.timers.values()].find(t=>t.ms===115000).fn();assert.equal(h.child.kills,1);[...h.timers.values()].find(t=>t.ms===5000).fn();const r=await h.pending;assert.equal(r.exitCode,1);assert.equal(r.receipt.cleanup,'UNVERIFIED');
});
