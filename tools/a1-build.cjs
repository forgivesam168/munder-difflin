...
async function buildCandidate() {
  ...
  const stage=path.join(root,'.tmp/a1-build-source');
  ...
  const candidate={sourceCommit:baseline,runId:binding.runId,sourceInputs,overlaySha256:map(binding.overlay),adapterSha256:map(binding.adapterFiles),
    build:{command:'node tools/a1-build.cjs',mode:'production',sourceDirectory:'.tmp/a1-build-source',write:'memory bundle then exclusive artifact files',envFile:false,posthogKey:'empty',
    ...
    syntheticProject:{root:'.tmp/a1-native-001/project',files:{'readme.txt':binding.hash('A1 synthetic read\n')}},
    invocation:{executable:'node_modules/electron/dist/electron.exe',entry:'.tmp/a1-native-001/artifact/main/index.js',args:['--munder-controlled-read']},
    timeouts:{app:60000,bootstrap:30000,workload:70000,cleanup:10000,outer:115000,fallback:5000},
    receipt:{requestVersion:1,resultVersion:1,supervisorVersion:1,sequence:['deny','reload','allow','read','display','reload','allow','read','display'],finish:'normal close'},
