...
async function buildCandidate() {
  ...
  const stage=path.join(root,'.tmp/a1-build-source-004');
  ...
  const candidate={sourceCommit:baseline,runId:binding.runId,sourceInputs,overlaySha256:map(binding.overlay),adapterSha256:map(binding.adapterFiles),
    build:{command:'node tools/a1-build.cjs',mode:'production',sourceDirectory:'.tmp/a1-build-source-004',write:'memory bundle then exclusive artifact files',envFile:false,posthogKey:'empty',
    ...
    syntheticProject:{root:'.tmp/a1-native-004/project',files:{'readme.txt':binding.hash('A1 synthetic read\n')}},
    invocation:{executable:'node_modules/electron/dist/electron.exe',entry:'.tmp/a1-native-004/artifact/main/index.js',args:['--munder-controlled-read']},
