...
$run = Join-Path $repository '.tmp/a1-native-003'
...
if ($permit.version -ne 1 -or $permit.runId -cne 'a1-native-003' -or $permit.candidateSha256 -cne $payloadHash -or
    $request.version -ne 1 -or $request.runId -cne 'a1-native-003' -or $request.candidateSha256 -cne $payloadHash -or
