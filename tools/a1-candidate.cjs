...
const baseline = '980a51b4726aee4e50e8e9840bc0e710f5d4b345';
const contract = require('../src/shared/a1-contract.json');
const runId = contract.runId;
const run = path.join(root, '.tmp', runId);
const manifestFile = path.join(root, 'tasks/a1-admission-candidate-007.json');
...
function verify({ fresh = true } = {}) {
  ...
  assert.equal(c.runId, runId); assert.deepEqual(c.invocation, { executable: 'node_modules/electron/dist/electron.exe', entry: '.tmp/a1-native-007/artifact/main/index.js', args: ['--munder-controlled-read'] });
  validateTimeouts(c);
