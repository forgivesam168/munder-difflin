...
const baseline = '9b65dbf826c5068da1de559b5b61c14938b0fb08';
const contract = require('../src/shared/a1-contract.json');
const runId = contract.runId;
const run = path.join(root, '.tmp', runId);
const manifestFile = path.join(root, 'tasks/a1-admission-candidate-003.json');
...
function verify({ fresh = true } = {}) {
  ...
  assert.equal(c.runId, runId); assert.deepEqual(c.invocation, { executable: 'node_modules/electron/dist/electron.exe', entry: '.tmp/a1-native-003/artifact/main/index.js', args: ['--munder-controlled-read'] });
  validateTimeouts(c);
