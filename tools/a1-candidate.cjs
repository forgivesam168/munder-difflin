...
const baseline = 'eab8602d2b0c369f3ee89bfbc18415c25be8c131';
const contract = require('../src/shared/a1-contract.json');
const runId = contract.runId;
const run = path.join(root, '.tmp', runId);
const manifestFile = path.join(root, 'tasks/a1-admission-candidate-004.json');
...
function verify({ fresh = true } = {}) {
  ...
  assert.equal(c.runId, runId); assert.deepEqual(c.invocation, { executable: 'node_modules/electron/dist/electron.exe', entry: '.tmp/a1-native-004/artifact/main/index.js', args: ['--munder-controlled-read'] });
  validateTimeouts(c);
