...
export interface SpawnOptions {
   ...
   *  it. Ingestion authenticates it in `spawnAgentCore`, which either refuses
   *  it or replaces it with a separately minted bounded preparation. `spawn`
   *  refuses the field before command resolution or any generic spawn effect,
   *  so a bypassing caller cannot reach a generic path through it. */
...
export class PtyManager {
  ...
  spawn(opts: SpawnOptions, owner: WebContents | null = null): { ok: boolean; error?: string } {
    ...
    // Only spawnAgentCore may transform a preflight using its private bridge.
    // This layer accepts the resulting bounded capability, never the preflight.
    // Refuse before bounded dispatch and before every generic spawn effect.
